...
while (true) {
  let userInput = prompt(
    "what do you want to do in our bank\n" +
      "1. login\n" +
      "2. Sign Up\n" +
      "3. Exit ",
  );
      ...
      while (true) {
        ...
        if (BankMenu === "5") {
          ...
          if (deposit > 0) {
            ...
            customerStatement[emailIndex].push("Deposited Rs :" + deposit);
          ...
          if (withDraw <= customerBalance[emailIndex] && withDraw > 0) {
            ...
            customerStatement[emailIndex].push("WithDraw Rs :" + withDraw);
            alert("your new balance is" + customerBalance[emailIndex]);
          } else {
          ...
          if (
            transferAmount > 0 &&
            transferAmount <= customerBalance[emailIndex]
          ) {
            customerBalance[emailIndex] -= transferAmount;
            customerStatement[emailIndex].push(
              "transferred Rs :" + transferAmount + "To" + receiveEmail,
            );
            customerBalance[receiveIndex] += transferAmount;
            customerStatement[receiveIndex].push(
              "Received Rs :" +
                transferAmount +
                "from" +
                customerEmail[emailIndex],
            );
            alert(
              "Transferred : " +
                transferAmount +
                ...
                receiveEmail +
                "\nYour new balance is: " +
                customerBalance[emailIndex] +
                "\nReceiver's balance: " +
                customerBalance[receiveIndex],
            );
          } else {
            alert("Invalid balance");
          }
        } else if (BankMenu === "6") {
          if (customerStatement[emailIndex].length === 0) {
            alert("No transaction found");
            continue;
          }
          let allStatement = "";
          let i = 0;
          while (i < customerStatement[emailIndex].length) {
            allStatement += customerStatement[emailIndex][i] + "\n";
            i++;
          }
          alert(allStatement);
        }
